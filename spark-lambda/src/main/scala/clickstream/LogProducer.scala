...
object LogProducer extends App {
  ...
  val Products = scala.io.Source.fromInputStream(getClass.getResourceAsStream("/products.csv")).getLines().toList
  val Referrers = scala.io.Source.fromInputStream(getClass.getResourceAsStream("/referrers.csv")).getLines().toList
  ...
  val incrementTimeEvery = rnd.nextInt(math.min(wlc.records, 100)) + 1
